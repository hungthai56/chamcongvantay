		...
		if (isset($this->filecache[$file])) {
			...
			if (is_file($file) && is_readable($file)) {
				...
				if ($file_text === false) {
					...
					$file_text = "<b>__Template fatal error: file [$file] does not exist in the include path__</b>";
				} elseif ($this->debug) {
					$file_text = '<!-- Template debug: ' . realpath($file) . ' (via include path) -->' . "\n" . $file_text;
				}
			} elseif (!is_file($file)) {
				...
				$file_text .= "<b>__Template fatal error: file [$file] is not readable__</b>";
			}
