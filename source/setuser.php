...
function setuser($link, $manv, $tencd, $password, $maychamcong)
{
    ...
    $datanew = '{"PIN": "' . $manv . '","Name": "' . $tencd . '","Pri": "0","Passwd": "","Card": "","Grp": "","TZ": "","ViceCard": ""}';
    // echo $datanew;
    $curl = curl_init();
    ...
    curl_setopt_array(
        $curl,
        array(
            CURLOPT_URL => 'http://nhandulieu.ovn.vn/templates/process.php?type=3&idlenh=1',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
            ...
            CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
            CURLOPT_CUSTOMREQUEST => 'POST',
            CURLOPT_POSTFIELDS => $datanew,
            CURLOPT_HTTPHEADER => array(
                'Content-Type: application/json',
                'Cookie: PHPSESSID=3m7s4jik60ku8qle7t3fq3sfr2'
            ),
        )
    );
...
function settem($manv){

    $settem = '{ "PIN": "' . $manv . '","FID": 1,"RETRY": "3","OVERWRITE": "1"}';
    // echo $settem;
    $curl = curl_init();
    curl_setopt_array($curl, array(
        CURLOPT_URL => 'http://nhandulieu.ovn.vn/templates/process.php?type=3&idlenh=2',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
...
if ($_POST["btnUpdate"] != "") {
    ...
    $date = date("Y-m-d");
    $sql = "SELECT Count(ID) as Count from nhanvien1";
    $Count = getdong($sql);
    $Count = $Count['Count'] + 1;
    $rec = "";
    if ($_GET["id"] == "-1") {
        $sql = "insert into nhanvien1 set linkcc='$link', IDnhanvien='$manv', uid='$manv', user_id='$manv', tencd='$tencd', role='$role', password='$password', idcc='$maychamcong', IDcuahang='$cuahang', ngaytao='$date', tinhtrang = 1,  Ten='$nhanviendl',  MaNV='$manvdl'";
        $update = $data->query($sql);
        $rec = setuser($link, $manv, $tencd, $password, $maychamcong);
        $get = "SELECT uid from nhanvien1 order by ID desc limit 1 ";
        $check = getdong($get);
        // echo $check['uid'];
        // return;
        if ($check['uid'] == $manv) {
        ...
        $sql = "UPDATE nhanvien1 set linkcc='$link', IDnhanvien='$manv', uid='$manv', user_id='$ID', tencd='$tencd', role='$role', password='$password', idcc='$maychamcong', IDcuahang='$cuahang', Ten='$nhanviendl',  MaNV='$manvdl' where uid='$ID'";
        // echo $sql;
        $update = $data->query($sql);
...
if ($_GET["Del"] != "") {
    $IDD = $_GET["Del"];
    $sql = "delete from nhanvien1  where ID = " . $IDD . "";
    // echo $sql;
    // return;
    $update = $data->query($sql);
    $xoa = true;
    ...
    if ($_REQUEST["id"] == "" || $them || $xoa) {
        ...
        $sql = "SELECT a.ID, a.template,a.user_id,a.uid,a.IDnhanvien, a.tencd , a.role ,c.tenmay, c.link, c.cong, a.ngaytao, a.Ten, a.MaNV from nhanvien1 as a  
        ...
        //$SOST = 0 ;
        // phan trang===================================================================
        $sql = "SELECT ID FROM nhanvien1 where 1=1 and tinhtrang = 1 order by tencd ";
        // echo $sql;
        ...
        if ($_REQUEST["id"] == "-1") {
            ...
            $sql = "SELECT a.ID,a.user_id,a.uid,a.IDnhanvien, b.MaNV, b.Ten,a.tencd , a.role ,c.tenmay, a.idcc, a.password from nhanvien1 as a 
